import sys
from .network import ASSET_TESTNET, NEON_DEVNET

def special_check(network):
    ...
    if network == NEON_DEVNET:
        print("neon evm does not support 0x05 precompile, you need to switch to fork")
        print("https://github.com/Diffuse-fi/p256-verifier")
        print('commit "replace 0x05 precompile call with solidity implementation"')
        while(True):
            user_input = input("If you haven't done it yet, do it and launch this script again. Resume? [y/n]: ").lower()
            if user_input == 'y':
                break
